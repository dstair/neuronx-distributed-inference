...
"""Unit tests for GLM-5.1 inference configuration.

CPU-only tests that validate config parsing, MLA parameter setup,
MoE routing config, DSA indexer params, and FP8 dequantization logic.
"""
...
def _make_config(**overrides):
    """Create a Glm5InferenceConfig with reasonable defaults matching GLM-5.1."""
    ...
    config = Glm5InferenceConfig(neuron_config=neuron_config, **defaults)
    return config
...
class TestConfigParsing(unittest.TestCase):
    ...
    def test_qk_head_dim_computed(self):
        """qk_head_dim should be qk_nope_head_dim + qk_rope_head_dim = 256."""
        config = _make_config()
        self.assertEqual(config.qk_head_dim, 256)

    def test_head_dim_override_for_kv_cache(self):
        """MLA overrides head_dim to rope_dim + kv_lora_rank + index_head_dim for KV cache."""
        config = _make_config()
        # 64 + 512 + 128 = 704
        self.assertEqual(config.head_dim, 704)
    ...
    def test_intermediate_size_swap(self):
        ...
        config = _make_config(intermediate_size=12288, moe_intermediate_size=2048)
        self.assertEqual(config.intermediate_size, 2048)
        self.assertEqual(config.dense_intermediate_size, 12288)
...
class TestDSAConfig(unittest.TestCase):
    """Test DSA (Dynamic Sparse Attention) indexer configuration."""

    def test_has_indexer_always_true(self):
        """GLM-5.1 always has DSA indexer on every layer."""
        config = _make_config()
        self.assertTrue(config.has_indexer)

    def test_indexer_params(self):
        config = _make_config()
        self.assertEqual(config.index_n_heads, 32)
        self.assertEqual(config.index_head_dim, 128)
...
class TestNeuronConfig(unittest.TestCase):
    ...
    def test_neuron_config_cls(self):
        self.assertEqual(
            Glm5InferenceConfig.get_neuron_config_cls(),
            Glm5NeuronConfig,
        )
